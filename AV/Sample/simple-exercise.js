var samplePRO = (function() {
    ...
    function setState() {
        ...
        if (state.lastState) {
            var index;
            for (var i = 0; i < state.lastState.ind.length; i++) {
                index = state.lastState.ind[i];
                if (index.cls) {
                    console.dir(i + 1);
                    // simulate student clicks
                    // TODO: should find a better way to do this, to be part of JSAV API?
                    // $(".jsavindex:nth-child(" + (i + 1) + ") .jsavvaluelabel").trigger("click");
                } else {
                    break;
                }
            };
