...
(function($) {
  ...
  function getLog(that) {
    if (!PARAMS.no_dump) {
      return that._jsondump()
    }
    return ""
  }
  ...
  ep.reset = function() {
    if (!this.initialStructures) {
      ...
      if (!PARAMS.reset_url && !PARAMS.submit_url) {
        window.parent.postMessage(JSON.stringify({
          type: "jsav-reset",
          data: {
            seed: seed,
...
$(document.body).on("jsav-log-event", function(evt, data) {
  if (data.type === "jsav-exercise-init" && PARAMS.resizeiframe !== "false") {
    var jsavContainer = $("#container");
    if (jsavContainer.length === 0) {
      jsavContainer = $("#jsavcontainer")
    }
